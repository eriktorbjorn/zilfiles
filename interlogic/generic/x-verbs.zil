...
<ROUTINE NO-NEED ("OPTIONAL" (STR <>) (OBJ <>))
	<SETG CLOCK-WAIT T>
	<TELL !\( CHE+V ,WINNER "do" "n't need to ">
...
<CONSTANT YAWNS <LTABLE 0 "unusual" "interesting" "extraordinary" "special">>
...
<ROUTINE NOT-HOLDING? (OBJ)
	<COND (<AND <NOT <IN? .OBJ ,WINNER>>
	       ...
	       <SETG CLOCK-WAIT T>
	       <TELL !\( CHE+V ,WINNER "is" " not holding" HIM .OBJ ".)" CR>)>>
...
<ROUTINE ITAKE ("OPTIONAL" (VB T) "AUX" CNT OBJ L)
	 ...
	 <COND (<NOT <FSET? ,PRSO ,TAKEBIT>>
		...
		<TOO-BAD-BUT>
		<TELL
THE .OBJ " slips from" HIS ,WINNER " arms while" HE+V ,WINNER "is" " taking"
HIM ,PRSO ", and both tumble " <GROUND-DESC> ". " CHE+V ,WINNER "is"
...
<CONSTANT WHO-CARES-VERB
	<PLTABLE "do" "do" "let" "seem">>

<CONSTANT WHO-CARES-TBL
...
<ROUTINE PRE-ANALYZE ()
 <COND (<ROOM-CHECK>
	<RTRUE>)
       (<OR <FSET? ,PRSO ,ACTORBIT> ;<EQUAL? ,PRSO ,YOU ,ME>>
	<SETG CLOCK-WAIT T>
...
<ROUTINE V-ANALYZE ()
 <COND ;(<EQUAL? ,PRSI ,FINGERPRINTS>
	...
	<RTRUE>)
       (<FSET? ,PRSO ,ACTORBIT> <TELL "How?" CR>)
       ;(<FSET? ,PRSO ,LIGHTBIT> <CHECK-ON-OFF>)
       (<FSET? ,PRSO ,DOORBIT> <CHECK-DOOR ,PRSO>)
       (T <TELL CHE+V ,PRSO "look" " normal." CR> ;<YOU-CANT "check">)>>
...
<ROUTINE WAITING-FOR-YOU-TO-SPEAK ()
	<TELL CHE+V ,PRSO "seem" " to be waiting for you to speak." CR>>
...
<ROUTINE PRE-ASK-CONTEXT-ABOUT ("OPTIONAL" (V 0) "AUX" P)
 ...
 <COND (<QCONTEXT-GOOD?>
	...
	<RTRUE>)
       (<SET P <FIND-FLAG-HERE-NOT ,ACTORBIT ,MUNGBIT ,WINNER>>
	<TELL-I-ASSUME .P " Ask">
	<PERFORM .V .P ,PRSO>
	<RTRUE>)>>
...
<ROUTINE PRE-ASK-CONTEXT-FOR ("AUX" P)
 <COND (<FSET? <SET P <LOC ,PRSO>> ,ACTORBIT>
...
<ROUTINE V-DIAGNOSE ()
 <COND (<T? ,PRSO> <YOU-CANT ;"diagnose">)
       (T <TELL CHE+V ,WINNER "is" " wide awake and in good health." CR>)>>

<ROUTINE TELL-NOT-IN (OBJ)
	<SETG CLOCK-WAIT T>
	<TELL !\( CHE+V ,WINNER "is" " not in" HIM .OBJ "!)" CR>>
...
<ROUTINE V-THROUGH ("AUX" RM DIR)
	<COND (<AND <NOUN-USED? ,W?DOOR ;,W?GATE ;,W?HOLE>
		    ;<FSET? ,PRSO ,DOORBIT>
	      ...
	      (<NOT <FSET? ,PRSO ,TAKEBIT>>
	       <TELL CHE+V ,WINNER "bang" " into" THE ,PRSO>
	       <THIS-IS-IT ,PRSO>
...
<ROUTINE V-EXAMINE ("AUX" (TXT <>))
	 <COND (<OR <==? ,PRSO ,PSEUDO-OBJECT>
		    <AND <NOUN-USED? ,W?DOOR ;,W?DOORS ;,W?KEYHOLE>
			 <GLOBAL-IN? ,PRSO ,HERE>>>
		<SET TXT T>)>
	 <COND (<DOBJ? INTDIR>
		<SETG CLOCK-WAIT T>
...
<ROUTINE GLOBAL-IN? (OBJ1 OBJ2 "AUX" TBL)
	 <COND (<EQUAL? .OBJ1 .OBJ2>
		...
		<ZMEMQ/B .OBJ1 .TBL <RMGL-SIZE .TBL>>)>>
...
CHE ,WINNER " can't give " A ,PRSO " to " A ,PRSI "!" CR>)
	       ;(<FSET? ,PRSI ,MUNGBIT>
		<TELL CHE+V ,PRSI "do" "n't respond." CR>)
	       (<IOBJ? PLAYER>
		<PERFORM ,V?TAKE ,PRSO>
		<RTRUE>)
	       (T
		<TELL "Politely," THE ,PRSI " refuses your offer." CR>)
	       ;(T
		<MOVE ,PRSO ,PRSI>
		<TELL CHE+V ,PRSI "accept" " your gift." CR>
		<RTRUE>)>>
...
<ROUTINE V-HELLO () ;("OPTIONAL" (HELL T))
 <COND (<FSET? ,PRSO ,ACTORBIT> ;<GETP ,PRSO ,P?CHARACTER>
 ...
 <COND (<OR <FSET? ,PRSO ,DOORBIT>
	...
	<COND (<FSET? ,PRSO ,OPENBIT>
	      ...
	      ;(<AND <SET P <DOOR-ROOM ,HERE ,PRSO>>
		    <SET P <FIND-IN .P ,ACTORBIT ,PLAYER>>>
	       <FCLEAR ,PRSO ,LOCKED>
	       <FSET ,PRSO ,OPENBIT>
	       <FSET ,PRSO ,ONBIT>
	       <UNSNOOZE .P>
...
<ROUTINE V-SEARCH-FOR ()
	 <COND (<IN? ,PRSO ,ROOMS>
		...
		;<START-SEARCH>)
	       (<FSET? ,PRSO ,ACTORBIT>
		<COND (<IN? ,PRSI ,PRSO>
		       <TELL "Indeed," HE+V ,PRSO "has" HIM ,PRSI "." CR>)
		      (T
...
THE ,PRSI " hidden on" HIS ,PRSO " person." CR>)>)>
		...
		<COND ;(<FSET? ,PRSI ,SECRETBIT>
		       <DISCOVER ,PRSI>)
		      (T <TELL
"How observant you are! There" HE+V ,PRSI "is" "!" CR>)>)
...
<ROUTINE IF-SPY ()
	;<COND (<NOT <FSET? ,PRSO ,ACTORBIT>> <TELL "break">)
	      (T <TELL "drop">)>
	<COND (<ZERO? ,PRSI>
...
<ROUTINE V-SMELL ()
	<TELL CHE+V ,PRSO "smell" " just like " A ,PRSO "!" CR>>
...
<ROUTINE PRE-TAKE-WITH ("AUX" X)
	 <COND (<DOBJ? YOU>
		...
		<SETG CLOCK-WAIT T>
		<TELL !\( CHE+V ,WINNER "is" " in" HIM ,PRSO ", nitwit!)" CR>)>>

<ROUTINE V-TAKE ()
	 <COND (<==? <ITAKE> T>
		<TELL CHE+V ,WINNER "is" " now holding" THE ,PRSO "." CR>)>>
...
<ROUTINE V-TAKE-OFF ()
	 <COND ;(<DOBJ? NOW-WEARING>
		...
		<RTRUE>)
	       (<FSET? ,PRSO ,WORNBIT>
		<FCLEAR ,PRSO ,WORNBIT>
		<TELL "Okay," HE+V <LOC ,PRSO> "is" " no longer wearing">
		<MOVE ,PRSO ,WINNER>
		<TELL HIM ,PRSO "." CR>)
	       (T
		<TELL CHE+V <LOC ,PRSO> "is" "n't wearing" HIM ,PRSO "!" CR>)>>
...
"You don't need to take" HIM ,PRSO " out to use" HIM ,PRSO "." CR>)
	       ;(<==? <LOC ,PRSO> ,POCKET>
		<MOVE ,PRSO ,WINNER>
		<TELL CHE+V ,WINNER "is" " now holding" HIM ,PRSO "." CR>)
...
<ROUTINE OWN-FEET ()
	 ...
	 ;<COND (<==? ,WINNER ,PLAYER>
		<SETG PLAYER-SEATED <>>)>
	 <TELL CHE+V ,WINNER "is" " on" HIS ,WINNER " own feet again." CR>
	 <RTRUE>>
...
<ROUTINE PRE-TELL-ABOUT ("AUX" P)
 <COND (<DOBJ? PLAYER ;PLAYER-NAME>
	<COND (<QCONTEXT-GOOD?>
	      ...
	      (<AND <SET P <FIND-FLAG-HERE-NOT ,ACTORBIT ,MUNGBIT ,WINNER>>>
	       <TELL-I-ASSUME .P " Ask">
...
<ROUTINE V-THROW-AT ()
	 ...
	 <COND ;(<AND <FSET? ,PRSI ,ACTORBIT>
		...
		<TELL CHE+V ,PRSI "duck">)
	       (T <TELL CHE+V ,PRSI "do" "n't duck">)>
...
<ROUTINE V-UNLOCK ()
	 <COND (<OR <FSET? ,PRSO ,DOORBIT>
		...
		<COND (<NOT <FSET? ,PRSO ,LOCKED>>
	       ...
	       (T
		<SETG CLOCK-WAIT T>
		<TELL !\( CHE+V ,PRSO "is" "n't locked!)" CR>)>>
...
;<ROUTINE NO-CHANGING? ("AUX" X)
 ...
 ;<REPEAT ()
	 <COND (<ZERO? .X> <RETURN>)
		...
		<RETURN>)
	       (T <SET X <NEXT? .X>>)>>
 <COND (<SET X <FIND-FLAG-HERE-NOT ,ACTORBIT ,MUNGBIT ,WINNER>>
