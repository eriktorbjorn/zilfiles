...
;--------------------------------------------------------------------------
; @CGA_Write: Store a block of 8 pixels into display memory.  Trashes DI.
;	      Counts bytes written, watches for picture width to be hit,
;	      and advances to the next line.
;-------------------------------------------------------------------------
@CGA_Write Macro
	Local	CGAW_2,CGAW_1,CGAW_0
	Cmp	CGA_Shift,0		;Faster if we're byte-aligned
	 Jne	CGAW_0
	Les	DI,Tempbuff
	StoSB
	Inc	Word Ptr Tempbuff
	Cmp	DI,CGA_End		;End of line?
	 Jl	CGAW_2
	Mov	DI,CGALStart		;Yes, advance to next
	Add	DI,80
	Add	CGA_End,80
	Mov	Word Ptr Tempbuff,DI	;All done
	Jmp	CGAW_2
CGAW_0:	Mov	CL,CGA_Shift		;Current stuff ends up low in AL;
	...
	Les	DI,Tempbuff
	StoSB				;Store a byte
	Inc	Word Ptr Tempbuff	;Advance the pointer
	Cmp	DI,CGA_End		;Are we at end-of-line?
	 Jl	CGAW_2			;Nope, done
	Mov	AL,ES:[DI]		;Yes, pick up the next byte from the screen
	Mov	DI,CGA_Shift		;So we can dump our partial byte
	And	AL,CGA_Masks[DI]
	Or	AL,AH
	Mov	DI,Tempbuff
	StoSB
	Mov	DI,CGALStart		;Now advance to the next line
	Add	DI,80
	Add	CGA_End,80
	Mov	Tempbuff,DI
	Mov	AL,ES:[DI]		;Pick up the first byte
	Mov	CL,CGA_Shift		;And save the high part
	Ror	AX,CL
	And	AH,CGA_Omasks[DI]
	Mov	CGA_Temp,AH
CGAW_2:
	EndM	
...
CGA_End	   dw	 0		    ;DI setting for end of line
CGA_Shift  db    0		    ;Shift byte right by this much
...
Trivia_CGA:
	...
	Add	AX,CGALStart
	Mov	CGA_End,AX
	Mov	Word Ptr Tempbuff,DI	;Use the graphics segment as the output buf
	Mov	Word Ptr Tempbuff+2,GrafSeg
	Mov	CGA_Temp,0
	Cmp	AX,0
	 Je	C_LZ0
	Les	DI,Tempbuff
