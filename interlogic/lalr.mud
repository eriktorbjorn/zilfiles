...
<DEFINE PRINT-NONTERMINAL-STATE (VV:<OR TABLE FIX FALSE> SV:VECTOR OUTCHAN:CHANNEL)
  <COND
   (<TYPE? .VV TABLE>
    <REPEAT (IT)
...
<DEFINE RES-L=? (RES1:FIX RES2:FIX "AUX" RED1 RED2)
  <COND (<AND <G? .RES1 ,ACTION-SPLIT>
	 ...
	 <COND (<==? <REDUCTION-PRIORITY
		...
		<G=? <REDUCTION-SIZE .RED1>
		     <REDUCTION-SIZE .RED2>>)
	       (T
		<G=? <REDUCTION-PRIORITY .RED1> <REDUCTION-PRIORITY .RED2>>)>)
	(<COMPILATION-FLAG-VALUE "OLD-GRAMMAR-SORTING">
	 <L=? .RES1 .RES2>)
	(T
	 ; "Used to be L=?.  This makes reductions precede shifts"
	 <G=? .RES1 .RES2>)>>
