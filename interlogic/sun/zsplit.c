...
int game_version = 0;
char *picture_detail_file = 0;
char *picture_source_file = 0;
char *pic_segs = 0;
int global_pix = 0;
...
char		**envp;		/* Environment vector */
{
  ...
  for(i = 1; i < argc; ++i) {
	...
	break;
      case 'n':
	strcpy(output_name, aptr);
	strcpy(old_output_name, aptr);
	i = n;
	have_output_name = 1;
	printf("Output files will be named %s.\n", output_name);
	break;
      case '0':
	seg0_pix = 1;
	...
	break;
      case 'v':
	picture_detail_file = aptr;
	++n;
	picture_source_file = argv[n];
	i = n;
	printf("Picture information from %s; pictures from %s.\n",
	       picture_detail_file, picture_source_file);
	break;
      case 'u':
...
int mapest;
{
  ...
  else if (picture_detail_file) {
    if ((allowed_disks > 4) || ((allowed_disks == 4) && (seg0_pix)))
      get_picture_data(picture_detail_file, &global_pix);
    else
      get_picture_data(picture_detail_file, 0); }
  for (i = 0; i < numsegs; i++)
    display_segment(i);
  if (close(segfd) < 0) {
  ...
  close(zipfd);
  game_version = (objfile[2] << 8) | objfile[3];
...
char *temppages;
{
  ...
  for (i = first; i < last; i++) {
    ...
    printf("%d (%d) left on %d.\n", left_on_disk(&disks[i]),
	   amt(disks[i].pages), i + 1); }
  for (i = 0; i < numsegs; i++) {
...
int s0_disk;
{
  ...
  if (left_on_disk(&disks[s0_disk]) > 0) {
	...
	if (amount > 0) {
	  ...
	  if (amount <= cur_left) {
	    got_some = i;
	    flush_from(amount, s0_disk, s0_disk + 1, allowed_disks, temppages, 0);
	    ...
	    if (!got_some)
	      got_some = i;
	    amount = 0;
	    flush_from(amount, s0_disk, s0_disk + 1, allowed_disks,
		       temppages, cur_left);
	    printf("%d left on %d.\n", left_on_disk(&disks[s0_disk]),
		   s0_disk + 1); } } } } }
  return(got_some); }
...
split()
{
  ...
  int curseg;          /* Disk we are currently looking for adjacents to */
  int fillseg;         /* Disk we are looking for adjacents to while filling */
  int sts = 0;
  int i, j, k, pno, bno, bestdisk, bestleft, templeft, tempcode;
  int pages_left;
  ...
  if ((active_disks > 1) && allowed_disks) {
	...
	disks[i].pix += dup_segment->seg_pic_size;
	dup_segment->seg_disk_mask |= 1 << i;
    ...
    if (seg0_pix) {
      ...
      segments[0].seg_disk_mask |= 1 << seg0_disk;
	...
	if (++times_through > allowed_disks) {
	  if (left_on_disk(&disks[seg0_disk]) > 0) {
	    ...
	    bestdisk = -1;
	    for (i = seg0_disk + 1; i < allowed_disks; i++) {
	      templeft = left_on_disk(&disks[i]);
	      tempcode = amt(disks[i].pages);
	      if (tempcode > cur_left)
		tempcode = cur_left;
	      if ((templeft + tempcode) > bestleft) {
		bestleft = templeft + tempcode;
		bestdisk = i; } }
		...
		printf("Moving page %d from %d (%d (%d) left) to %d (%d left).\n",
		       i, bestdisk + 1, left_on_disk(&disks[bestdisk]) + 1,
		       amt(disks[bestdisk].pages) - 1,
		       seg0_disk + 1,
	...
	/* Add on segment 0 */
	sts = curseg = addnseg(disknum);         /* Find a new segment */
	if (sts < numsegs)
	  any_on_disk = 1;
	while(sts < numsegs)
	  if ((sts = addnaseg(disknum,curseg)) >= numsegs)  /* Fit adjacent segments */
	    if ((sts = curseg = newcurseg(disknum,curseg)) >= numsegs) 
	      sts = curseg = addnseg(disknum); /* No adjacent; try another new seg */
...
/* First try to add the startup segment, # 1. If it is unused and it fits, add it. */
  ...
  if (bestseg < numsegs) {
    ...
    segments[bestseg].seg_disk_mask |= 1 << disknum;
     ...
     int disknum,curseg;
{
  ...
  if (curseg >= numsegs)
    return(numsegs);		/* Can't be anything adjacent */
     ...
     int mapest;
{
  ...
  int start_lose;
  int rewrite_map = 0;
  int onamelen = strlen(output_name);
  long tmp;
  char *map;
  short *cmap;

  output_name[onamelen] = NULL;
  if (picture_detail_file) {
    strcat(output_name, ".d "); }
  else
    strcat(output_name,".ds ");        /* filename "template" */
  onamelen = strlen(output_name) - 1;
  ...
  cmap = (short *)map;
  cmap = &cmap[MAP_HEADER_SIZE]; /* Map entry for this disk */
  for (i = 0; i < numdisks; i++) {  /* Write the disks */
    output_name[onamelen] = '0' + i + 1;  /* Open the specific disk file */
    if ((dskfd = open(output_name,O_WRONLY|O_CREAT|O_TRUNC,0666)) < 0) {
    ...
    if (i == 0) {
      if (!vpreload) {
	do_preload_resource(preload);
	do_vers_resource(game_version, old_output_name);
	write_a_res(output_name, "ZdbA", 0, 1); }
	...
	rewrite_map = 1;
	tmp = lseek(dskfd, 0, 1);
	cmap[DH_GLOBAL_PIX] = tmp / PAGESIZE; /* Global picture directory */
	gd = construct_global_directory();
	/* Don't forget to checksum the directory */
	for (k = 0; k < global_pix * PAGESIZE; k++)
	  foo += ((unsigned char *)gd)[k];
	cmap[DH_CHECKSUM] += foo;
	write(dskfd, (char *)gd, global_pix * PAGESIZE); }
      if (disks[i].pix > 0) {
	rewrite_map = 1;
	tmp = lseek(dskfd, 0, 1);
	cmap[DH_PICTURE_PAGE] = tmp / PAGESIZE;
	output_picture_file(dskfd, i, picture_source_file); } }
    cmap = &cmap[DISK_HEADER_SIZE + cmap[DH_BLOCKS] * BLOCK_SIZE];
    close(dskfd); }
  if (rewrite_map) {
    output_name[onamelen] = '1';
    dskfd = open(output_name, O_RDWR, 0666);
    write(dskfd, map, mappages * PAGESIZE);
    close(dskfd); }
  start_lose = -1;
  for (j = 0; j <= top_page; j++) {
